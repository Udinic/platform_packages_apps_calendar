...
package com.android.calendar_udinic.event;
...
import com.android.calendar_udinic.CalendarEventModel;
import com.android.calendar_udinic.CalendarEventModel.Attendee;
import com.android.calendar_udinic.CalendarEventModel.ReminderEntry;
import com.android.calendar_udinic.EmailAddressAdapter;
import com.android.calendar_udinic.EventInfoFragment;
import com.android.calendar_udinic.GeneralPreferences;
import com.android.calendar_udinic.R;
import com.android.calendar_udinic.RecipientAdapter;
import com.android.calendar_udinic.TimezoneAdapter;
import com.android.calendar_udinic.TimezoneAdapter.TimezoneRow;
import com.android.calendar_udinic.Utils;
import com.android.calendar_udinic.event.EditEventHelper.EditDoneRunnable;
...
public class EditEventView implements View.OnClickListener, DialogInterface.OnCancelListener,
    ...
    private static final String GOOGLE_SECONDARY_CALENDAR = "calendar_udinic.google.com";
    ...
    /**
     * Does prep steps for saving a calendar_udinic event.
     *
     ...
     * exists and has a calendar_udinic it can be associated with, either because it's
     * an existing event or we've finished querying.
     *
     * @return false if there is no model or no calendar_udinic had been loaded yet,
     * true otherwise.
     */
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (dialog == mNoCalendarsDialog) {
            ...
            if (which == DialogInterface.BUTTON_POSITIVE) {
                ...
                final String[] array = {"com.android.calendar_udinic"};
     ...
     * needed for the current set of reminders and calendar_udinic properties, and then creates UI
     * elements.
     */
    private void prepareReminders() {
        ...
        // lists (which may have elements added and removed based on calendar_udinic properties).  This
        ...
        // Remove any reminder methods that aren't allowed for this calendar_udinic.  If this is
        ...
        if (model.mHasAlarm) {
            ...
            // from the provider, even if the count exceeds the calendar_udinic maximum.  (Also, for
    ...
    public void setModel(CalendarEventModel model) {
        ...
        if (model.mUri != null) {
            // This is an existing event so hide the calendar_udinic spinner
            // since we can't change the calendar_udinic.
     ...
     * events allow you to select a calendar_udinic while editing an event.
     * <p>
     * We tuck a reference to a Cursor with calendar_udinic database data into the spinner, so that
     * we can easily extract calendar_udinic-specific values when the value changes (the spinner's
    ...
    // Find the calendar_udinic position in the cursor that matches calendar_udinic in
    // preference
    private int findDefaultCalendarPosition(Cursor calendarsCursor) {
        ...
        while (calendarsCursor.moveToNext()) {
            ...
            if (defaultCalendar == null) {
                ...
                // Found the default calendar_udinic.
                return position;
            }
            position++;
        }
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        // calendar_udinic selection changes or on screen rotation
        Cursor c = (Cursor) parent.getItemAtPosition(position);
        if (c == null) {
            // TODO: can this happen? should we drop this check?
            Log.w(TAG, "Cursor not set on calendar_udinic item");
            return;
        }
        ...
        // Update the max/allowed reminders with the new calendar_udinic properties.
